...
export default function Index() {
  ...
  const { todayMarks, lastMark, loading: attendanceLoading } = useAttendance();
  ...
  return (
    <AppLayout>
      <div className="space-y-6 animate-fade-in">
        {/* Welcome Section */}
        <div className="p-6 rounded-2xl" style={{ background: 'var(--gradient-hero)' }}>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          ...
          {/* Mark Attendance CTA */}
          {!isRest && (
            <Card className="group hover:shadow-lg transition-shadow cursor-pointer" onClick={() => navigate('/attendance')}>
          ...
          {/* Rest Schedule */}
          <Card className="group hover:shadow-lg transition-shadow cursor-pointer" onClick={() => navigate('/rest-schedule')}>
            <CardContent className="p-6 flex items-center justify-between">
              <div className="flex items-center gap-4">
                  ...
                  >
                    <span
                      className={`w-2 h-2 rounded-full ${
                        mark.mark_type === 'IN' ? 'bg-success' : 'bg-primary'
                      }`}
                    />
                    ...
                    <span className="text-muted-foreground">
                      {format(new Date(mark.timestamp), 'HH:mm')}
                    </span>
                  </div>
                ))}
              </div>
        ...
        {(role === 'department_head' || role === 'global_manager') && (
          <Card>
            <CardHeader>
            ...
            <CardContent>
              <div className="flex flex-wrap gap-3">
                ...
                {role === 'global_manager' && (
                  <>
                    <Button variant="outline" onClick={() => navigate('/global')}>
                      Panel global
                    </Button>
